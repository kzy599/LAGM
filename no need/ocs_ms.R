...
#' @param targetMinInbreedingPct target mating inbreeding percentage, normalized [0,100]; NULL = disabled (default, behaviour unchanged); 0 = truncation selection without avoidance; 100 = minimum achievable inbreeding / maximum avoidance; 75 = balanced default point; only meaningful on the one-step minInbreedingMating=FALSE path
#' @param coancestryWeight weight on deviation from the targeted coancestry level; NULL = AlphaMate default (-1.0: negative penalizes above-target coancestry, positive encourages higher); non-NULL writes CoancestryWeight
#' @param inbreedingWeight weight on deviation from the targeted inbreeding level; NULL = AlphaMate default (-1.0: negative penalizes above-target inbreeding, positive encourages higher); non-NULL writes InbreedingWeight
#' @param coancestryWeightBelow logical; TRUE writes CoancestryWeightBelow , Yes (also penalize solutions BELOW the coancestry target; AlphaMate default No)
#' @param inbreedingWeightBelow logical; TRUE writes InbreedingWeightBelow , Yes (also penalize solutions BELOW the inbreeding target; AlphaMate default No)
#'
#' @export
ocs <- function(pop, nCrosses, usePed = FALSE, ped_parents = NULL,
                ...
                MateAllocation       = TRUE,
                minInbreedingMating  = FALSE,
                targetMinInbreedingPct = NULL,
                coancestryWeight = NULL,
                inbreedingWeight = NULL,
                coancestryWeightBelow = NULL,
                inbreedingWeightBelow = NULL) {   # ← 新增
  ...
  if (!is.null(targetMinInbreedingPct))    cat("TargetMinInbreedingPct , ", targetMinInbreedingPct, "\n")
  if (!is.null(coancestryWeight))          cat("CoancestryWeight , ", coancestryWeight, "\n")
  if (!is.null(inbreedingWeight))          cat("InbreedingWeight , ", inbreedingWeight, "\n")
  if (!is.null(coancestryWeightBelow) && coancestryWeightBelow) cat("CoancestryWeightBelow , Yes\n")
  if (!is.null(inbreedingWeightBelow) && inbreedingWeightBelow) cat("InbreedingWeightBelow , Yes\n")
