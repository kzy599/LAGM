...
anova_model<- aov(gg0~app,data = rep_out[gen == nG,])
summary(anova_model)
...
P<- ggplot(data = zt1,aes(x=gen,y=value,group = app, color = app))+
  ...
  geom_errorbar(aes(ymin=value-valuese,
                    ymax=value+valuese),
                width=0.05,alpha = 0.5)+labs(color="Mat")+scale_color_aaas()+scale_x_continuous(limits = c(1,nG),breaks = seq(1,nG,5))
ggsave("Figrue_gg_line.pdf", P , width = 15, height = 6, dpi = 300)


plotDT = function(pv,pvse,dt){
app_all = unique(dt$app)
output = data.table()
app_v = c()
type_v = c()
value_v = c()
...
for(t in pv){
  ...
  for(a in app_all){
    ...
    value_v = c(value_v,dt[app==a,..t][[1]])
    valuese_v = c(valuese_v,dt[app==a,..sd_temp][[1]])
    app_v = c(app_v,a)
    type_v = c(type_v,t)
  }
  ...
  output[,app:=app_v]
  output[,type:=type_v]
  output[,value:=value_v]
  output[,valuese:=valuese_v]
return(output)
}

pv = c("gg0","inb_ped","inbEff_ped")
pvse = paste(pv,"_sd",sep = "")
ck_dt = plotDT(pv = pv,pvse = pvse,dt = results[gen ==nG,])
ck_dt
ck_dt[type=="gg0",type:="Gain"]
ck_dt[type=="inb_ped",type:="Inbreeding"]
ck_dt[type=="inbEff_ped",type:="Efficiency"]
ck_dt$type = factor(ck_dt$type,level = c("Gain","Inbreeding","Efficiency"))
ck_dt = ck_dt[app!="rl",]
ck_dt$app = factor(ck_dt$app,level = c("tc","ocsrate","ocs25","ocs45","ocs65","ocs90","ran","rl2"))
...
rep = data.table()
programs = c("5_generation","10_generation","15_generation","20_generation")
dir = c("/home/kangziyi/RLmating/Gen5","/home/kangziyi/RLmating/Gen10","/home/kangziyi/RLmating/Gen15","/home/kangziyi/RLmating/Gen20")
original_dir = getwd()
for(p in c(1:4)){
  setwd(dir[p])
for(i in c(61:80)){
  ...
  temp[,rep:=i]
  genic0=mean(temp$genic/(1-(temp$lost_diversity)/100))
#   temp$gain_std = temp$gain_std*sqrt(genic0)
  temp$gain_std = temp$gain_std*sqrt(genic0)
  temp$gain = temp$gain_std*genic0
  temp$lost_diversity = (1-sqrt(temp$genic)/sqrt(genic0))*100
...
# 为每个 app × rep × programs 组合创建一行 gen0 数据
origin_rows <- dat %>%
  distinct(app, rep, programs) %>%
  mutate(
    gen = 0,
    lost_diversity = 0,
    gain_std = 0,
    # 其他列设为 NA（不参与分析）
    gv = NA_real_, genetic = NA_real_, genic = NA_real_,
    inb = NA_real_, Ne = NA_real_, inb_ped = NA_real_
  )

# 合并原始数据与原点
dat_full <- bind_rows(dat, origin_rows) %>%
  arrange(app, programs, rep, gen)

cat("原始数据行数:", nrow(dat), "\n")
cat("加入原点后行数:", nrow(dat_full), "\n")
...
library(emmeans)   # 备选：更直观的 Tukey 后验
library(tidyverse)
library(broom)
library(rlang)
library(patchwork)
...
for(i in c(41:60)){
  ...
  temp$gain_std = temp$gain/sqrt(genetic_diversity0)
  temp$gain_std_genic = temp$gain/sqrt(genic_diversity0)
...
# dat_temp = data.table()
# for(g in c(15,10,5)){
#   dat_temp1 = dat_full[gen %in%c(0:g),]
#   dat_temp1$programs = paste0(g,"_generation")
#   dat_temp = rbind(dat_temp1,dat_temp)
# }
# dat_full = rbind(dat_temp,dat_full)
...
# }
# dat_full = rbind(dat_temp,dat_full)
# dat_temp = dat[dat$programs!="nhp_20_generation",]
# dat_temp = dat_temp[!(dat_temp$app%in%c("rl2","rl")),]
# dat_temp = as.data.table(dat_temp)
# dat_full = rbind(dat_full,dat_temp)
library(data.table)
library(ggplot2)
library(MASS)        # rlm: robust linear model
# 可选: library(robustbase)  # lmrob: 更强的稳健回归
...
colors     = c(LAGM1   = "#1f3b73",  LAGM2   = "#d62728",
                   ...
                   GOCS90 = "#8b0000",  Random  = "#999999",
                   LAGM5 = "#79465e", LAGM7 = "#930c83",TC = "#0c937a")
colors = c(
        # --- LAGM 组：冷色阶渐变（窗口越长，颜色越深邃）---
        "LAGM1"   = "#7bccc4",  # 浅青水绿 (Window = 1)
        "LAGM2"   = "#43a2ca",  # 晴空蓝   (Window = 2)
...
plot_dt[plot_dt$app=="LAGM7"]
plot_dt[plot_dt$app%in%c("GOCS45","TC","GOCS25")]
P = plot_tradeoff_arrow(
    plot_dt[plot_dt$app%in%c("GOCS45","TC","LAGM5")],
    x_var  = "gen",
    y_var  = "genic",
    # progs  = "10 gen",
    # x_reverse = FALSE,
  ...
  y = "Pedigree Inbreeding"    # 修改 y 轴标题
)+theme(strip.background = element_rect(fill = NA, color = NA))

ggsave("Figure S99.pdf", P , width = 15, height = 10, dpi = 300,device = cairo_pdf)
...
ggsave("Figure 9.pdf", combined_plot , width = 15, height = 8, dpi = 300,device = cairo_pdf)

# ============================================================
# 示例 2：只看 3 个 app，显示拟合线
# ============================================================

plot_dt = plot_dt[app%in%c("OCS45","OCS65","LAGM7","LAGM3")]
P = plot_tradeoff_arrow(
    plot_dt,
    x_var    = "gen",
    y_var    = "inb",
...
ggsave("Figrue_inb_line.pdf", P , width = 15, height = 6, dpi = 300,device = cairo_pdf)
...
# 基本用法
res1 <- fit_robust_slopes(dat_full, y = "gain_std_genic", x = "lost_diversity_genic")
res1 <- fit_robust_slopes(dat_full, y = "gain", x = "lost_diversity_He")
print(res1$rep_slopes)
print(res1$slope_summary)

# 换一组指标:gain ~ lost_He
res2 <- fit_robust_slopes(dat_full, y = "gain", x = "lost_diversity_He")

# 换一组指标:gain ~ lost_Ho
res3 <- fit_robust_slopes(dat_full, y = "gain", x = "lost_diversity_Ho")

# 一次跑多组并合并
combos <- list(
    c("gain", "inb"),
    c("gain", "inb_ped"),
    c("gain", "lost_diversity_He"),
    c("gain", "lost_diversity_Ho")
)
all_slopes <- rbindlist(lapply(combos, function(p) {
    fit_robust_slopes(dat_full, y = p[1], x = p[2])$slope_summary
}))
print(all_slopes)
all_slopes[programs=="10_generation"&x_var=="inb",]


# ---- 方法 A: 经典 aov + TukeyHSD,逐个 programs 跑 ----
library(data.table)

dat_full <- as.data.table(dat_full)
dat_temp <- copy(dat_full)
dat_temp = dat_temp[app%in%c("ocs25","ocs45","ocs65","rl2"),]
setDT(dat_temp)

# programs 名 -> 该 programs 下要看的 gen
gen_map <- c("5_generation"  = 5,
             "10_generation" = 10,
             "15_generation" = 15,
             "20_generation" = 20)

# 只保留每个 programs 对应末代的数据
dat_temp <- dat_temp[programs %in% names(gen_map) &
                     gen == gen_map[programs]]

tukey_by_program <- dat_temp[, {
    if (length(unique(app)) < 2 || .N < 3) {
        list(result = list(NULL))
    } else {
        aov_fit <- aov(gain~ app, data = .SD)
        list(result = list(TukeyHSD(aov_fit, "app")))
    }
}, by = programs]

# 按 5/10/15/20 顺序打印
tukey_by_program <- tukey_by_program[
    order(match(programs, names(gen_map)))
]

for (i in seq_len(nrow(tukey_by_program))) {
    p   <- as.character(tukey_by_program$programs[i])
    res <- tukey_by_program$result[[i]]
    cat("\n================ programs =", p,
        " (gen =", gen_map[p], ") ================\n")
    if (is.null(res)) {
        cat("数据不足,跳过\n")
...
dF_loglin <- compute_per_gen_rates(dat_full[gen%in%c(0:15),],F_col="inb",G_col="gain")
dF_loglin <- compute_per_gen_rates(dat_full,F_col="inb",G_col="gain")
ck=dF_loglin$summary[,.(app,programs,dF_mean,dG_mean,eff_per_gen_mean,eff_cumulative_mean)]
ck
dF_loglin$per_rep
# 跨 rep 汇总
dF_loglin[, .(
  dF_mean = mean(dF_loglin, na.rm = TRUE),
  dF_se   = sd(dF_loglin,   na.rm = TRUE) / sqrt(.N),
...
res <- compute_per_gen_rates(
    ...
    terminal_cols = c("gv", "genetic", "genic", "He", "Ho", "Ne", "inb","inb_ped",
                    ...
                    "lost_diversity_Ho", "lost_diversity_genetic","gain_std_genic","lost_diversity_genic"),
...
plot_dt$dF_late = plot_dt$dF_late*100
plot_dt$dF_early = plot_dt$dF_early*100
y_var = "eff_per_gen"
...
ggsave("Figure 6.pdf", combined_plot ,
        width = 15, height = 8, dpi = 300,
        device = cairo_pdf)
ggsave("Figure 6_new.pdf", combined_plot ,
        width = 15, height = 8, dpi = 300,
        device = cairo_pdf)
