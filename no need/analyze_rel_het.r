...
zt1[app=="rel_rl2",app:="rel_LAGM7"]
zt1[app=="rel_rl2_3",app:="rel_LAGM3"]
zt1[app=="rel_rl2_5",app:="rel_LAGM5"]
zt1[app=="rel_rl2_5m",app:="rel_LAGM1"]
zt1[app=="rel_rl2_10m",app:="rel_LAGM2"]
return(zt1)
}
rep_out = data.table()
for(d in alldir){
...
if(dir[which(alldir == d)] == "gas20") nrep = c(31:50) # nrep = c(21:30,41:50)
for(r in nrep){
...
if(r %in% c(31:40)) output$app = paste("rel_",output$app,sep = "")
...
ck_dt$app = factor(ck_dt$app,level = c("LAGM7","rel_LAGM7","LAGM5","rel_LAGM5","LAGM3","rel_LAGM3","LAGM2","rel_LAGM2","LAGM1","rel_LAGM1"))
...
P <- ggplot(data = ck_dt[type == y_lab_name,], aes(x = app, y = value, group = app, fill = app)) +
  ...
  ggtitle("LAGM converison effociency (rel vs he-based))") +
  theme(
...
ggsave(paste("Figrue_bar_",y_lab_name,"lagm.pdf",sep = ""), P , width = 22, height = 10, dpi = 300)


ck_dt_line = rename_dt(ck_dt_line)
nG = 20
c("LAGM1","LAGM2","LAGM3","LAGM5","LAGM7","OCS45","OCS25")
P<- ggplot(data = ck_dt_line[type%in%c("Gain","Inbreeding")&app%in%c("LAGM1","LAGM2","LAGM3","LAGM7","rel_LAGM1","rel_LAGM2","rel_LAGM3","rel_LAGM7"),],aes(x=gen,y=value,group = app, color = app))+
  geom_point()+
  geom_line()+
...
ggsave("Figrue_gain_inbreeding_line_rel_27.pdf", P , width = 15, height = 6, dpi = 300)
